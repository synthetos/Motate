...
namespace Motate {
	...
	struct Pin /*: public PinBase*/ {
		...
		static const uint8_t portLetter; // set later
		static const uint8_t mask; // set later
	...
	#define _MAKE_MOTATE_PIN(pinNum, registerLetter, registerChar, registerPin)\
		...
		template <> const uint8_t Pin<pinNum>::portLetter = (uint8_t) registerChar;\
		template <> const uint8_t Pin<pinNum>::mask = (1 << registerPin);\
		template <> bool Pin<pinNum>::isNull() { return false; };\
	...
	template <> uint8_t Port8<registerChar>::getPins(const uint8_t mask) {\
		return (PIN ## registerLetter) & (mask);\
	}\
	typedef Port8<registerChar> Port ## registerLetter;\
	Port ## registerLetter port ## registerLetter;
	
	// Special NullPin and nullPin objects
	template <> void Pin<-1>::init(const PinSetupType) {};
