...
constexpr int c_strlen(const char* p, const int count_ = 0) {
    return !p ? 0 : ((*p == 0) ? count_ : c_strlen(p + 1, count_ + 1));
}
...
constexpr int c_strreverse(char* const t, const int count_, char hold = 0) {
    return count_ > 1
           ...
           ? (hold = *t, *t = *(t + (count_ - 1)), *(t + (count_ - 1)) = hold),
           c_strreverse(t + 1, count_ - 2), count_ : count_;
}
...
constexpr int c_itoa_backwards_(const int value, char* p, int maxlen, const int count_ = 0) {
    return (maxlen < 1) ? (maxlen == 1) ? (*p = 0, 0) : 0
                        : ((value > 0 || count_ == 0) ? (*p = (value % 10) + '0'),  // <- comma operator
                           c_itoa_backwards_((value / 10), p + 1, maxlen - 1, count_ + 1) : count_);
}
...
constexpr int c_itoa_len(int value, int length_ = 0) {
    return (value < 0)   ? c_itoa_len(-value, length_ + 1)
           : (value > 0) ? c_itoa_len(value / 10, length_ + 1)
                         : length_ + 1;
}
...
constexpr int c_strcpy(char* const t, const char* f, int max_len_, const int count_ = 0) {
    return max_len_ <= 0 || !t || !f ? count_ : max_len_ == 1 ? *t = 0, count_ + 1 : (*t = *f),  // <- comma operator
           ((*f == 0) ? count_ : c_strcpy(t + 1, f + 1, max_len_ - 1, count_ + 1));
}
...
struct c_strcpyx_t {
    ...
    constexpr int32_t get_length() const { return 0; };

    constexpr int32_t copy_to(char* const dest_, int32_t max_len_) const { return *dest_ = 0, 0; };
};
...
struct c_strcpyx_t<const char*, Ts...> : c_strcpyx_t<Ts...> {
    ...
    constexpr c_strcpyx_t(const char* value, Ts... ts)
        : c_strcpyx_t<Ts...>{ts...}, val_{value}, length_{c_strlen(val_)} {
        ;
    }

    constexpr int32_t get_length() const { return length_ + parent_t::get_length(); };

    constexpr int32_t copy_to(char* const dest_, int32_t max_len_, int32_t written_ = 0) const {
        return (written_ = c_strcpy(dest_, val_, max_len_),
                (max_len_ < get_length()) ? written_
...
struct str_buf {
    ...
    /*constexpr*/ bool record_copy_(int w) const { return ((w_ += w), (l_ > w_)); };
    ...
    constexpr bool copy(const char* f) const {
        return (l_ > w_) ? record_copy_(c_strcpy(b_ + w_, f, l_ - w_ - r_)) : false;
    };

    // Copy at most max_len_ characters, returning false if it failed
    constexpr bool copy(float f, uint8_t precision) const {
        return (l_ > w_) ? record_copy_zerofail_(c_floattoa(f, b_ + w_, l_ - w_ - r_, precision)) : false;
    };

    // Copy at most max_len_ characters, returning false if it failed
    template <typename int_type>
    constexpr bool copy(int_type i) const {
        return (l_ > w_) ? record_copy_zerofail_(c_itoa(i, b_ + w_, l_ - w_ - r_)) : false;
    };
...
inline uint32_t Rd_bits(volatile const uint32_t& value, uint32_t mask) { return ((value) & (mask)); };
inline uint32_t Wr_bits(volatile uint32_t& lvalue, uint32_t mask, uint32_t bits) {
    return ((lvalue) = ((lvalue) & ~(mask)) | ((bits) & (mask)));
};
inline bool     Tst_bits(volatile const uint32_t& value, uint32_t mask) { return (Rd_bits(value, mask) != 0); };
inline uint32_t Clr_bits(volatile uint32_t& lvalue, uint32_t mask) { return ((lvalue) &= ~(mask)); };
inline uint32_t Set_bits(volatile uint32_t& lvalue, uint32_t mask) { return ((lvalue) |= (mask)); };
inline uint32_t Tgl_bits(volatile uint32_t& lvalue, uint32_t mask) { return ((lvalue) ^= (mask)); };
